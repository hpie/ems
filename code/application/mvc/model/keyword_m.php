...
class keyword_m extends Models {
    ...
    public function getkeyword() {
        $q = "SELECT p.*,s.status_title,s.status_code FROM job_keyword p
     ...
     public function insert($params) {
        ...
        if ($columns) {
            $q = "INSERT INTO job_keyword($columns) values($values)";
     ...
     public function getKeyworddetail($id) {
        $q = "SELECT p.*,s.status_title,s.status_code FROM job_keyword p
            LEFT JOIN cdac_status s ON p.status=s.status_code WHERE key_id='$id'";
        $result = $this->query->select($q);
        if ($data = $this->query->fetch_array($result)) {
            return $data;
        }
    ...
    public function updatekeyword($params,$id) {       
        ...
        if ($columnsdesc) {
            $q = "UPDATE job_keyword SET $columnsdesc WHERE key_id =$id";
            print_r ($q) ;                
